...
- Each archive can be downloaded and uncompressed. The resulting folder contains: 
  ...
  - file ```SI_S.txt``` contains the data in the following format ```298.15    GR_25_4_1 m1_00_Vitamin_A-GFN1_003.xyz  0.03   290 173.29   0.79   9.37 183.45```. ```298.15```- is Temperature in K, ```GR_25_4_1``` - thermochemistry protocol, i.e. nsRRHO(t=25), ```m1_00_Vitamin_A-GFN1_003.xyz``` - filename of the most dG stable according to specific thermochemistry protocol structure, ```0.03``` - Boltzmann weight, ```290``` - number of conformers Nconf, ```173.29``` - entropy of the most stable structure according to SE method, ``0.79``` - entropy correction stemming from Boltzmann averaging', ```9.37``` - Gibbs-Shannon entropy, ```183.45``` - the sum of entropy of the most stable structure, Boltzmann averaging correction and Gibbs-Shannon entropy.
